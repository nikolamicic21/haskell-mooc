...
appendAll str strings = do
  readStr <- readIORef str
  let result = foldl (++) readStr strings
  writeIORef str result
...
hSelectLines h nums = do
  lines <- hFetchLines h
  return (selectLines lines nums 1 (length lines))
    where
      selectLines [] _ _ _ = []
      selectLines (line:lines) nums currentLine len
        | currentLine > len = []
        | currentLine `elem` nums = line : selectLines lines nums (currentLine + 1) len
        | otherwise = selectLines lines nums (currentLine + 1) len
...
interact' f state = do
  command <- getLine
  let (shouldContinue, message, newState) = f (command, state)
  putStrLn message
  if shouldContinue
