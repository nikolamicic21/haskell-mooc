...
instance Eq Name where
  (==) a b = case (a, b) of
    (Name nameA, Name nameB) ->
      map toLower nameA == map toLower nameB
...
instance Eq a => Eq (List a) where
  (==) a b = case (a, b) of
    (Empty, LNode rv rn) -> False
    (LNode lv ln, Empty) -> False
    (Empty, Empty) -> True
    (LNode lv ln, LNode rv rn) ->
      lv == rv && ln == rn
...
instance Price Milk where
  price milk = case milk of
    (Milk litres) -> 15 * litres
...
instance Price a => Price [a] where
  price = foldr (\x acc -> acc + price x) 0
...
simplify p = case p of
  (RationalNumber a b) ->
    RationalNumber (div a commonDenominator) (div b commonDenominator)
    where
      commonDenominator = gcd a b
...
instance Num RationalNumber where
  ...
  abs q = case q of
    (RationalNumber a b) -> RationalNumber (abs a) b
  signum q = case q of
    (RationalNumber a b) -> case signum (a * b) of
      -1 -> RationalNumber (-1) 1
      0 -> RationalNumber 0 1
      1 -> RationalNumber 1 1
  fromInteger x = RationalNumber x 1
  negate q = case q of
    (RationalNumber a b) -> RationalNumber (negate a) b
...
class Cycle a where
  ...
  stepMany :: Int -> a -> a
  stepMany 0 c = c
  stepMany num c = stepMany (num - 1) (step c)
