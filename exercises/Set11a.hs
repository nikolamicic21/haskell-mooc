...
readWords n = do
  wordsRead <- replicateM n getLine
  return (sort wordsRead)
...
readUntil f = do
  ...
  if f wordRead
    then return []
      ...
      readUntilReturn <- readUntil f
      return (wordRead:readUntilReturn)
...
countdownPrint n = do
  print n
  when (n > 0) (do countdownPrint (n - 1))
  ...
  where
    ...
    isums' sum n = do
      if n > 0
        then do
          nextNum <- getLine
          let nextSum = (sum + read nextNum::Int)
          print nextSum
          isums' nextSum (n - 1)
        else return sum
