...
instance Functor List where
  fmap f list = case list of
    (LNode a list) -> LNode (f a) (fmap f list)
    Empty -> Empty
...
instance Functor TwoList where
  fmap f list = case list of
    (TwoNode v1 v2 list) -> TwoNode (f v1) (f v2) (fmap f list)
    TwoEmpty -> TwoEmpty
...
count x = foldr (\v acc -> if v == x then acc + 1 else acc) 0
...
inBoth a b =
  foldr
    ( \bX acc ->
        if bX `elem` a && notElem bX acc
          then bX : acc
          else acc
    )
    (foldr (\aX acc -> if aX `elem` b then aX : acc else acc) [] a)
    b
...
instance Foldable List where
  foldr f acc list = case list of
    (LNode nodeVal nextNode) -> f nodeVal (foldr f acc nextNode)
    Empty -> acc
...
instance Foldable TwoList where
  foldr f acc twoList = case twoList of
    (TwoNode nodeVal1 nodeVal2 nextNode) -> f nodeVal1 (f nodeVal2 (foldr f acc nextNode))
    TwoEmpty -> acc
...
instance Functor Tree where
  fmap f tree = case tree of
    (Node nodeVal leftNode rightNode) -> Node (f nodeVal) (fmap f leftNode) (fmap f rightNode)
    Leaf -> Leaf

sumTree :: Monoid m => Tree m -> m
sumTree tree = case tree of
  (Node nodeVal leftNode rightNode) -> sumTree leftNode <> nodeVal <> sumTree rightNode
  Leaf -> mempty
