...
genList = do
  size <- choose (3, 5)
  list <- vectorOf size (chooseInt (0, 10))
  return (sort list)
...
numberArgGen :: Gen Arg
numberArgGen = do
  num <- chooseInt (0, 10)
  return (Number num)

variableArgGen :: Gen Arg
variableArgGen = do
  var <- elements "abcxyz"
  return (Variable var)

instance Arbitrary Arg where
  arbitrary = oneof [numberArgGen, variableArgGen]

plusExprGen :: Gen Expression
plusExprGen = do
  arg1 <- oneof [numberArgGen, variableArgGen]
  arg2 <- oneof [numberArgGen, variableArgGen]
  return (Plus arg1 arg2)

minusExprGen :: Gen Expression
minusExprGen = do
  arg1 <- oneof [numberArgGen, variableArgGen]
  arg2 <- oneof [numberArgGen, variableArgGen]
...
instance Arbitrary Expression where
  arbitrary = oneof [plusExprGen, minusExprGen]
