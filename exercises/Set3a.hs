...
multiCompose f
  | null f = id
  | otherwise = head f . multiCompose (tail f)
...
multiApp2 :: ([b] -> d) -> [c -> b] -> c -> d
multiApp2 f gs value = f (map (\g -> g value) gs)

multiApp :: ([b] -> d) -> [c -> b] -> c -> d
multiApp f gs value = f (multiApp' gs value)

multiApp' :: [c -> b] -> c -> [b]
multiApp' gs value
  | null gs = []
  | otherwise = head gs value : multiApp' (tail gs) value
...
interpreter' (command : commands) x y result = case command of
  "up" -> interpreter' commands x (y + 1) result
  "down" -> interpreter' commands x (y - 1) result
  "left" -> interpreter' commands (x - 1) y result
  "right" -> interpreter' commands (x + 1) y result
  "printX" -> interpreter' commands x y (result ++ [show x])
  "printY" -> interpreter' commands x y (result ++ [show y])