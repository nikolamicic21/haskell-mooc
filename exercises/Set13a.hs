...
split str = if length wordList < 2
    then Nothing
    else Just (head wordList, head $ tail wordList)
  where
    wordList = words str
...
checkNumber (for, sur) = if any isDigit for || any isDigit sur
  then Nothing
  else Just (for, sur)
...
count x =
  modify
    ( \it -> case lookup x it of
        Nothing -> it ++ [(x, 1)]
        Just val -> (x, val + 1) : delete (x, val) it
    )
...
occurrences [] = do
  state <- get
  keys <- mapM (return . fst) state
  return (length . nub $ keys)
occurrences (x:xs) = do
  val <- occurrences xs
  state <- get
  case lookup x state of
    Nothing -> do
      modify (\currState -> (x, 1) : currState)
      return (val + 1)
    Just currVal -> do
      modify (\currState -> (x, currVal + 1) : delete (x, currVal) currState)
      return val
