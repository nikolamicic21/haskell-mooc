...
prettyPrint size queens = prettyPrint' (1, 1) queens size
  where
    prettyPrint' (row, col) stack dim
      | row > dim || col > dim = ""
      | row <= dim && col == dim = point ++ "\n" ++ prettyPrint' (nextRow (row, col)) stack dim
      | otherwise = point ++ prettyPrint' (nextCol (row, col)) stack dim
      where
        point = if (row, col) `elem` stack then "Q" else "."
...
danger candidate =
  any
    ( \x ->
        sameRow x candidate || sameCol x candidate || sameDiag x candidate || sameAntidiag x candidate
    )
...
prettyPrint2 size queens = prettyPrint' (1, 1) queens size
  where
    prettyPrint' (row, col) stack dim
      | row > dim || col > dim = ""
      | row <= dim && col == dim = point ++ "\n" ++ prettyPrint' (nextRow (row, col)) stack dim
      ...
      where
        point
          | (row, col) `elem` stack = "Q"
          | danger (row, col) stack = "#"
          | otherwise = "."
...
fixFirst n (queen:s) = fixFirst' n queen s
  where
    fixFirst' n (queenRow, queenCol) s
      | queenCol > n = Nothing
      | danger (queenRow, queenCol) s = fixFirst' n (queenRow, queenCol + 1) s
      | otherwise = Just ((queenRow, queenCol):s)
...
continue [] = [(1, 1)]
continue (firstQueen:restOfStack) =
  nextRow firstQueen : firstQueen : restOfStack

backtrack :: Stack -> Stack
backtrack [] = [(1, 1)]
backtrack [x] = []
backtrack (firstQueen:secondQueen:restOfStack) =
  nextCol secondQueen : restOfStack
...
finish n (firstStack:stack) =
  if length (firstStack:stack) > n then stack
  else finish n (step n (firstStack:stack))
